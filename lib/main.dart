...
*/
      ...
      Navigator.push(
        ...
        MaterialPageRoute(builder: (context) => OAuthWeb(
          ...
          components: const {
            'scope': 'ZohoBooks.invoices.CREATE,ZohoBooks.invoices.READ,ZohoBooks.invoices.UPDATE,ZohoBooks.invoices.DELETE,ZohoBooks.settings.READ,ZohoBooks.contacts.READ',
            'client_id': client_id,
            'state': 'testing',
            'response_type': 'code',
      ...
      var response = await oAuthGet('https://accounts.zoho.com/oauth/v2/token',
      {
        'code' : code,
        'client_id': client_id,
      ...
      if (response.containsKey('access_token'))
      {
        ...
        final zoho = ZohoClient(clientId:client_id,  clientSecret: client_secret, redirectUrl: 'https://abcalc.8u.cz/test');
        ...
        final cr  = await zoho.get('https://books.zoho.com/api/v3/invoices', requestType: ZohoRequest_Type.POST, 
                          jsonData: {
...
Uri buildUri(String url, [Map<String, String>? components])
{
  ...
  {
    builder.write('?');
    builder.writeAll(components.entries.map((e) => '${Uri.encodeComponent(e.key)}=${Uri.encodeComponent(e.value)}'), '&');
  }
...
class ZohoClient
{
  ...
  Future<Map> get(String url, {ZohoRequest_Type requestType=ZohoRequest_Type.GET, Map<String, String>? components, dynamic jsonData}) async
  {
    final uri = buildUri(url, components); 
    final client = HttpClient();
